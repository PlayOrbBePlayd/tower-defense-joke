...
(function (global) {
  ...
  function migrate(s) {
    ...
    if (!merged.banks || !Array.isArray((merged.banks || {}).jeop) || !merged.banks.jeop.length) {
  ...
  const Store = {
    ...
    // Switch the active Jeopardy ('jeop') or Family Feud ('feud') board.
    // The current board's edits are saved back to its bank slot first, then
    // the target bank is copied in and round-state resets for a fresh game.
    switchBoard(kind, i) {
      Store.patch((s) => {
        const banks = s.banks && s.banks[kind];
        if (!banks || !banks[i]) return;
        const cur = s.activeBoards[kind] || 0;
        if (kind === 'jeop') {
          if (banks[cur]) banks[cur] = { name: banks[cur].name, categories: s.questions.jeopardy.categories, final: s.questions.jeopardy.final };
          s.activeBoards.jeop = i;
          const b = clone(banks[i]);
          s.questions.jeopardy = { categories: b.categories, final: b.final };
          s.jeop = clone(DEFAULT_STATE.jeop);
        } else {
          if (banks[cur]) banks[cur] = { name: banks[cur].name, questions: s.questions.main };
          s.activeBoards.feud = i;
          s.questions.main = clone(banks[i].questions);
          s.main.questionIndex = 0;
          initRound(s);
