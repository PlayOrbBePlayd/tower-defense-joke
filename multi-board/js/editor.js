...
(function () {
  ...
  $('edBoardSel').onchange = () => {
    ...
    render();
    toast('✎ Now editing: ' + S().banks[kind][S().activeBoards[kind]].name);
  };
  ...
  $('restoreBtn').onclick = () => {
    if (confirm('Replace ALL questions with the built-in samples?')) {
      Store.patch((s) => {
        s.questions = JSON.parse(JSON.stringify(window.FF_DEFAULT_QUESTIONS));
        s.banks = JSON.parse(JSON.stringify(window.FF_BANKS));
        s.activeBoards = { jeop: 0, feud: 0 };
      });
