...
(function () {
  ...
  function renderBoardPickers() {
    const s = S();
    const jp = $('jpBoardSel'), ff = $('ffBoardSel');
    if (jp && document.activeElement !== jp) {
      jp.innerHTML = (s.banks.jeop || []).map((b, i) =>
        `<option value="${i}" ${i === s.activeBoards.jeop ? 'selected' : ''}>${escHtml(b.name)}</option>`).join('');
    }
    if (ff && document.activeElement !== ff) {
      ff.innerHTML = (s.banks.feud || []).map((b, i) =>
        `<option value="${i}" ${i === s.activeBoards.feud ? 'selected' : ''}>${escHtml(b.name)}</option>`).join('');
    }
  }
  $('jpBoardSel').onchange = () => {
    Store.switchBoard('jeop', +$('jpBoardSel').value);
    Store.patch((st) => { st.boardMode = 'jeopardy-title'; });
    renderJpGrid(); Sound.click();
    toast('🟦 ' + S().banks.jeop[S().activeBoards.jeop].name + ' loaded');
  };
  $('ffBoardSel').onchange = () => {
    Store.switchBoard('feud', +$('ffBoardSel').value);
    Store.patch((st) => { st.boardMode = 'feud-title'; });
    renderMain(); Sound.click();
    toast('🔵 ' + S().banks.feud[S().activeBoards.feud].name + ' loaded');
  };
  renderBoardPickers();
